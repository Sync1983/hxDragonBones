...
class E4X 
{
	...
	private static inline var DESC_SHORTCUT:String = "_";
	private static inline var DESC_METHOD:String = "desc";
	private static inline var ATTR_METHOD:String = "a";
	private static inline var TEXT_METHOD:String = "text";
		...
		if (wrapInfo.wrapped && !isE4XFinalAccess(expr)) {
			...
			switch(returnType) {
				case Str:
					if(wrapInfo.type != null) {
						switch(wrapInfo.type) {
							case Node, IndNode: return macro E4X.doGetNodeNames($expr);
							case Attribute:		return macro E4X.doGetAttribs($expr);
							case Text:			return macro E4X.doGetText($expr);
						}
					} else {
				...
				case Boolean:
					if(wrapInfo.type != null) {
						switch(wrapInfo.type) {
							case Node, IndNode: return macro E4X.doHasNodes($expr);
							case Attribute:		return macro E4X.doHasAttribs($expr);
							case Text:			return macro E4X.doHasText($expr);
						}
					} else {
				...
				default:
					if(wrapInfo.type != null) {
						switch(wrapInfo.type) {
							case Node, IndNode: return macro E4X.doRetNodes($expr);
							case Attribute:		return macro E4X.doRetAttribs($expr);
							case Text:			return macro E4X.doRetText($expr);
						}
					} else {
	...
	private static function checkParam(expr:Expr, filterType:FilterType, wrapInfo:WrapInfo):Expr {
			...
			case EConst(c):
				switch(c) {
					case CString(s), CIdent(s):
						switch(filterType) {
							case Node, IndNode: expr = macro { return xml.nodeType == Xml.Element && xml.nodeName == $expr; };
							case Attribute: 	expr = macro { return attName == $expr; };
							case Text:			expr = macro { return text == $expr; };
						}
					default:
		...
		switch(filterType) {
			case IndNode:	return macro function(xml:Xml, _i:Int):Bool $expr;
			case Node:		return macro function(xml:Xml):Bool $expr;
			case Attribute:	return macro function(attName:String, attVal:String, xml:Xml):Bool $expr;
			case Text:		return macro function(text:Null<String>, xml:Xml):Bool $expr;
		}
	}
